...
pub async fn listen_for_price_changes(
  ...
  println!("LISTENING FOR CRYPTO PRICE CHANGES >= {}% in <= {}sec ...\n", target_percent_change, timeframe_in_seconds);
  ...
  loop {
      ...
      alert_on_price_changes(webhook_url, &mut initial_data, &current_data, target_percent_change).await;
      refresh_price_data(&mut initial_data, &current_data, timeframe_in_seconds); 
  }
}

fn refresh_price_data(initial_data: &mut CryptoPairsList, current_data: &CryptoPairsList, interval_seconds: u64) {
  initial_data.items.iter_mut().for_each(|(key, initial_pair)| {
      if initial_pair.alert_triggered 
          || is_time_passed(initial_pair.timestamp, current_data.items[key].timestamp, interval_seconds) {
