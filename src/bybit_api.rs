...
pub async fn get_price_data(
...
) -> Result<CryptoPairsList, Box<dyn std::error::Error>> {
    ...
    let timestamp = response_body["time"]
        .as_u64()
        .ok_or("Failed to parse timestamp")?;
    let pairs_list = response_body["result"]["list"]
        .as_array()
        .ok_or("Failed to parse pairs list")?;
